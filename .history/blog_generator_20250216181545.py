...
class BlogGenerator:
    def __init__(self):
        ...
        # Initialize Gemini 2.0 Pro Experimental
        genai.configure(api_key=os.getenv('GEMINI_API_KEY'))
        self.model = GenerativeModel('gemini-pro-experimental-02-05')
        
        # Configure model parameters for better blog generation
        self.generation_config = {
            'temperature': 0.7,  # Balance between creativity and consistency
            'top_p': 0.8,
            'top_k': 40,
            'max_output_tokens': 8192  # Maximum for longer blog posts
        }
    ...
    def _call_llm(self, prompt: str) -> str:
        """
        Call Gemini 2.0 Pro Experimental API to generate content.
