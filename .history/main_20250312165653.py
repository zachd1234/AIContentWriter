...
async def email_webhook(request: EmailWebhookRequest, x_api_key: str = Header(None)):
    ...
    try:
        ...
        print(f"Body: {request.body_plain[:100]}...")  # Print first 100 chars
        
        # Process the email (store in database, send notification, etc.)
        # For example, you could send a notification to yourself:
        
        # Option: Send to Discord webhook
        # import requests
        # discord_webhook_url = "YOUR_DISCORD_WEBHOOK_URL"
        # discord_payload = {
        #     "content": f"📬 New Email: {request.subject}\nFrom: {request.sender}\n\n{request.body_plain[:500]}..."
        # }
