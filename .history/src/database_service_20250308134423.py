...
class DatabaseService:
    ...
    def get_blog_info(self, site_id: int) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary containing blog information (name, url, description)
        """
        conn = None
        try:
            conn = self.get_connection()
            ...
            cursor.execute(
                "SELECT name, url, description FROM blog_info WHERE site_id = %s LIMIT 1",
                (site_id,)
            )
            ...
            if result:
                logger.info(f"Found blog info for site ID {site_id}: {result['name']}")
                return result
            else:
                logger.warning(f"No blog info found for site ID {site_id}")
                return {"name": f"Blog {site_id}", "url": "", "description": ""}
