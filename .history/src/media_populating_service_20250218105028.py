...
- Limit to 1-2 videos maximum to avoid overwhelming the reader"""
    ...
    def populate_media_in_html(self, html_content: str) -> str:
        ...
        try:
            ...
            print("🔍 Raw media suggestions:", media_json)  # Debug print
            ...
            for placement in media_placements:
                ...
                if media_type == 'image':
                    ...
                    clean_url = media_url.replace('https://ruckquest.com/wp-content/uploads/', '')
                    final_url = f"https://ruckquest.com/wp-content/uploads/{clean_url}"
                    media_html = f'<p><img src="{final_url}" alt="{placement.get("description", "")}"></p>'
                else:  # video
                    ...
                    if 'youtube.com' in media_url:
                        if 'watch?v=' in media_url:
                            ...
                            video_id = media_url.split('embed/')[-1]
                        else:
                            video_id = media_url.split('/')[-1]
                    else:
                        # If not a YouTube URL, skip this media
                        print(f"⚠️ Skipping non-YouTube video URL: {media_url}")
                        continue
