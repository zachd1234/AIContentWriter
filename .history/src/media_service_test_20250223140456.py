...
class PostWriterV2:
    ...
    def enhance_post(self, blog_post: str) -> str:
        ...
        try:
            ...
            MAKE SURE THAT THE JSON ARRAY ONLY CONTAINS THE MEDIA PLACEMENTS THAT YOU GENERATED.
            Do not include any media that wasn't successfully created using the tools above.
            """
            ...
            try:
                ...
                media_placements = json.loads(output_text)
                print(f"\n✅ Successfully parsed JSON with {len(media_placements)} media placements")
                for i, placement in enumerate(media_placements, 1):
                    print(f"\n📍 Media Placement {i}:")
                    print(f"  Type: {placement.get('mediaType', 'unknown')}")
                    print(f"  Insert Before: {placement.get('insertBefore', 'unknown')[:50]}...")
                    print(f"  Description: {placement.get('description', 'unknown')}")
                return output_text
