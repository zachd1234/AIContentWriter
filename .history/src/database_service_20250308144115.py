...
if __name__ == "__main__":
    ...
    # Test pop_next_urls with site IDs 1, 2, and 3
    print("\nTesting pop_next_urls:")
    try:
        for site_id in [1]:
            try:
                urls = db_service.pop_next_urls(5, site_id)
                print(f"Site ID {site_id} URLs popped: {len(urls)}")
                for url in urls:
                    print(f"  - {url['url']} (ID: {url['id']})")
            except Exception as e:
                print(f"Site ID {site_id}: {str(e)}")
    except Exception as e:
        print(f"Error testing pop_next_urls: {str(e)}")
