...
class DatabaseService:
    ...
    def get_blog_info(self, site_id: int) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary containing blog information (topic, description)
        """
        conn = None
        try:
            conn = self.get_connection()
            ...
            cursor.execute(
                "SELECT topic, description FROM blog_info WHERE site_id = %s LIMIT 1",
                (site_id,)
            )
            ...
            if result:
                ...
                return {"topic": f"Blog {site_id}", "description": ""}
