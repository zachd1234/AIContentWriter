...
def main():
    ...
    for email in test_emails:
        ...
        # Optional SMTP verification for valid emails
        if result['is_valid'] and input("\nRun SMTP verification? (y/n): ").lower() == 'y':
            print("Running SMTP verification (this may take a few seconds)...")
            smtp_result = validator.is_valid_email(email, check_smtp=True)
            print(f"SMTP Valid: {smtp_result['details'].get('smtp_valid', 'Not checked')}")
            print(f"SMTP Message: {smtp_result['details'].get('smtp_message', 'Not checked')}")
        
        # Ask to continue or exit
        if input("\nContinue testing? (y/n): ").lower() != 'y':
            break
