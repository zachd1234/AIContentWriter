...
class ProspectGenerator:
    ...
    def get_category_list(self, post_url: str, post_title: str, site_id: int = 1, num_urls: int = 5) -> Tuple[str, List[Dict[str, Any]]]:
        """
        Get the best category for a given post URL and title, then retrieve URLs from that category.
        
        Args:
            post_url: URL of the post
            post_title: Title of the post
            site_id: ID of the site to retrieve URLs for
            num_urls: Number of URLs to retrieve from the selected category
            
        Returns:
            Tuple containing (selected category, list of URL dictionaries)
        """
        ...
        try:
            ...
            3. Provide outreach diversity (avoid selecting the most obvious category unless it is the only logical match)
            ...
            # Now retrieve URLs from this category
            print(f"\n=== RETRIEVING URLS FROM CATEGORY: {best_category} ===")
            urls = self.db_service.get_urls_by_category(best_category, site_id, num_urls)
            
            if urls:
                print(f"Retrieved {len(urls)} URLs from category '{best_category}'")
                for i, url_data in enumerate(urls, 1):
                    print(f"{i}. {url_data['url']}")
            else:
                print(f"No URLs found for category '{best_category}' and site ID {site_id}")
            
            return best_category, urls
            
        except Exception as e:
            print(f"Error in get_category_list: {e}")
            # Return a default category and empty list if there's an error
            return None, []
...
def main():
    ...
    test_posts = [
        ...
        {
            "url": "https://ruckquest.com/the-ultimate-guide-to-rucking-gifts/",
            "title": "The Ultimate Guide to Rucking Gifts: Find the Perfect Present for Every Rucker"
        }
    ]
    
    for i, post in enumerate(test_posts, 1):
        print(f"\n=== TEST {i}: {post['title']} ===")
        best_category, urls = generator.get_category_list(post["url"], post["title"], site_id, 5)
        print(f"\nBest category: {best_category}")
        
        if urls:
            print(f"\nRetrieved {len(urls)} URLs from category '{best_category}':")
            for j, url_data in enumerate(urls, 1):
                print(f"{j}. {url_data['url']}")
        else:
            print(f"\nNo URLs found for category '{best_category}' and site ID {site_id}")
