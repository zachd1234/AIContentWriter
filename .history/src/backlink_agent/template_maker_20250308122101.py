...
class TemplateMaker:
    ...
    def _fetch_recent_post(self, site_url: str) -> Optional[str]:
        ...
        try:
            ...
            if posts and len(posts) > 0:
                ...
                valid_posts = [post for post in posts if not post['loc'].endswith('/blog/') and not post['loc'].endswith('/')]
    ...
    def _generate_personalized_email(self, site_url: str, variables: Dict[str, Any]) -> str:
        ...
        # Use the provided recent work URL or default to empty if not provided
        recent_work_url = variables.get('recent_work_url', target_recent_post)
        
        if not recent_work_url:
            logger.warning("No recent post found to use as recent work URL")
