...
{webpage_content}"""
    ...
    def generate_advanced_personalized_email_agentic(self, url: str, site_id: int) -> str:
        ...
        using an agentic approach with a single LLM call.
        ...
        try:
            ...
            # Create the agentic prompt that combines both tasks
            agentic_prompt = f"""You are an expert outreach strategist and email writer. Your task is to generate a personalized outreach email in two steps:

STEP 1: GENERATE AN OUTREACH ANGLE
First, identify ONE strong outreach angle that connects our blog topic to the target website's audience.

📌 **Given the following details:**
- **Our Blog:** {blog_name} - {blog_description}
- **Target Website:** {url} - {website_analysis}

🎯 **Outreach Angle Guidelines:**  
...
- Frame it as an opportunity for their audience (Why should their readers care?)
- If relevant, use a trend, statistic, or unique positioning to justify it.

The outreach angle should be a concise, 1-2 sentence statement (not a full email) that's punchy, casual, and relevant.

STEP 2: CRAFT THE OUTREACH EMAIL
Using the outreach angle you just created, craft a compelling outreach pitch email.
...
1️⃣ **Logical Pitch** → Present the outreach angle as your core argument, explaining why it matters to their audience.
   ...
   - Use the exact reasoning from your outreach angle, just phrased conversationally.  
...
OUTREACH ANGLE:
[Your 1-2 sentence outreach angle]

OUTREACH EMAIL:
[The full email body starting with the logical pitch]
...
"""
            ...
            agentic_model = GenerativeModel("gemini-2.0-flash-thinking-exp-01-21")
            
            # Log the prompt for debugging
            print(f"\nSending agentic prompt to Gemini API:\n{agentic_prompt[:200]}...\n")
            ...
            response = agentic_model.generate_content(
                ...
                generation_config=GenerationConfig(
                    temperature=0.7,
                    max_output_tokens=2048  # Ensure we have enough tokens for the full response
                )
            )
            ...
            # Extract just the outreach email part
            if "OUTREACH EMAIL:" in full_response:
                outreach_email = full_response.split("OUTREACH EMAIL:")[1].strip()
            else:
                # If the model didn't follow the format, return the whole response
                outreach_email = full_response
            
            # Log the extracted text
            print(f"\nExtracted outreach email: {outreach_email}\n")
