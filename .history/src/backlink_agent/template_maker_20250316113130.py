...
{webpage_content}"""
    ...
    def generate_advanced_personalized_email(self, url: str, site_id: int) -> str:
        ...
        try:
            ...
            # Generate the outreach angle first
            outreach_angle_prompt = f"""
You are an expert in content outreach strategy. Your task is to generate the strongest and most natural outreach angle that connects our blog topic to the target website's audience and content.
...
🎯 **Your Goal:**
- Find the most **natural, compelling connection** between their existing content and our blog.
- Frame our blog as a valuable addition to their website's audience.
- Avoid generic explanations—make the argument **specific to their site's mission and content focus.**

🔹 **Output Format:**
✅ A **concise, persuasive outreach angle** (2-3 sentences max).
✅ Keep it framed in **their audience's interests**, not just about our blog.
"""
            ...
            outreach_angle_response = model.generate_content(
            ...
            outreach_angle = outreach_angle_response.text.strip()
            logger.info(f"Generated outreach angle: {outreach_angle}")
            
            # Fetch one recent post from the target site to provide context for the AI
            target_recent_post = self._fetch_recent_post(url)
            target_recent_post_text = target_recent_post if target_recent_post else "No recent post found"
            ...
            # Create prompt for Gemini with the website analysis and outreach angle
            email_prompt = f"""
You are a professional outreach specialist. Create a personalized email to the owner of the blog at {url}.

Here's my analysis of their website:
{website_analysis}
...
Here's the specific outreach angle to use:
{outreach_angle}

Use this template structure, but incorporate the outreach angle and personalize it based on their blog content:

My name is {founder_name} and I am the founder of {blog_name}.

[Insert 1-2 sentences showing genuine appreciation for their specific content or approach based on the website analysis]

[Insert the outreach angle here]

Here's a sample of my most recent work: {recent_work_url}
...
IMPORTANT: 
1. Keep the introduction exactly as "I am the founder of {blog_name}"
2. Personalize the email based on the website analysis to show genuine interest in their content
3. Incorporate the outreach angle exactly as provided
4. Keep all other parts of the template exactly as shown
5. Return only the email body, no subject line
"""

            # Generate the email
            email_response = model.generate_content(
                email_prompt,
                generation_config=GenerationConfig(
                    temperature=0.7,
                    max_output_tokens=1024,
            ...
            # Return just the body text
            return email_response.text.strip()
