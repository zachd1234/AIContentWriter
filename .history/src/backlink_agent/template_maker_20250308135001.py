...
class TemplateMaker:
    ...
    def _generate_personalized_email(self, site_url: str, variables: Dict[str, Any]) -> str:
        ...
        blog_name = blog_info.get('name', '[Blog Name]')
...
if __name__ == "__main__":
    ...
    try:
        # Get the blog info from the database
        blog_info = template_maker.db_service.get_blog_info(variables["site_id"])
        
        # Print the blog info to verify
        print("\nBlog Information:")
        print(f"Blog Name: {blog_info.get('blog_name') or blog_info.get('topic')}")
        print(f"Blog URL: {blog_info.get('url')}")
        print(f"Blog Description: {blog_info.get('description')}")
        
        # Get the target site URL from the database
        variables["site_url"] = blog_info.get('url')
