...
class MockDatabaseService:
    def __init__(self):
        print("Initializing MockDatabaseService for testing")
    
    def find_recent_emails_by_recipient(self, recipient, site_id=None):
        print(f"MockDB: Finding recent emails for recipient: {recipient}")
        # Return a mock email record
        return [{
            'email_id': 'mock_email_123456',
            'recipient': recipient,
            'subject': 'Your outreach about fitness collaboration',
    ...
    def update_email_status(self, email_id, status, response_text=None):
        print(f"MockDB: Updating email {email_id} status to '{status}'")
        if response_text:
            print(f"MockDB: Response text: {response_text[:50]}...")
        return True
...
class EmailReplyProcessor:
    def __init__(self, testing=False):
        ...
        # Use mock database service for testing
        if testing:
            print("Using mock database service for testing")
