...
class ProspectGenerator:
    ...
    def generate_prospects_from_plan(self, prospect_plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns:
            The prospect plan with added website lists for each category
        """
...
def main():
    ...
    # Generate the prospect plan
    print("\n=== GENERATING PROSPECT PLAN ===")
    prospect_plan = generator.generate_prospect_plan(blog_title, blog_description)
    
    # Display the categories and their search terms
    print("\nCategories and Search Terms:")
    for i, category_obj in enumerate(prospect_plan["structured_categories"], 1):
        category = category_obj.get('category', 'Unknown Category')
        search_term = category_obj.get('search_term', 'No search term')
        print(f"{i}. {category}")
        print(f"   Search term: {search_term}")
    
    # Ask if user wants to proceed with searches
    proceed = input("\nDo you want to proceed with generating website lists? (y/n): ")
    
    if proceed.lower() == 'y':
        # Generate prospects from the plan
        print("\n=== GENERATING WEBSITE LISTS ===")
        results = generator.generate_prospects_from_plan(prospect_plan)
        
        # Display results
        print("\nResults by Category:")
        for i, category_data in enumerate(results["categories"], 1):
        ...
        print(f"\nTotal unique websites found: {results['total_websites']}")
