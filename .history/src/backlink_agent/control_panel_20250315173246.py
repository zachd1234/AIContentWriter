...
class ControlPanel:
    ...
    def run_advanced_outreach_campaign(self, site_id: int, post_url: str, post_title: str) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with campaign results
        """
        logger.info(f"Starting advanced outreach campaign for site ID: {site_id}")
        print(f"\n===== ADVANCED OUTREACH CAMPAIGN =====")
        print(f"Site ID: {site_id}")
        print(f"Post URL: {post_url}")
        print(f"Post Title: {post_title}")
        
        # Step 1: Get the best category and URLs for this post
        print(f"\n===== SELECTING CATEGORY AND PROSPECTS =====")
        ...
        logger.info(f"Selected category '{best_category}' with {len(prospects)} prospects")
        print(f"Selected category: {best_category}")
        print(f"Retrieved {len(prospects)} prospects")
        
        if not prospects:
            return {"status": "failed", "reason": f"No prospects available for category '{best_category}'", "emails_sent": 0}
        ...
        # Step 3: Create personalized emails with site_id and post information
        print(f"\n===== CREATING EMAILS WITH POST INFO =====")
        emails = self.email_creator.create_emails(
            prospect_urls, 
            site_id,
            post_url=post_url,
            post_title=post_title
        ...
        logger.info(f"Created {len(emails)} personalized emails with post information")
        print(f"Created {len(emails)} personalized emails")
        ...
        logger.info(f"Processed {len(prospects)} prospects with post: {post_title}")
        
        # Return campaign results
        return {
            "status": "completed",
            "category_used": best_category,
            "prospects_used": len(prospects),
            "emails_created": len(emails),
            "emails_sent": successful_sends,
            "emails_failed": failed_sends,
