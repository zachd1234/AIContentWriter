...
class TemplateMaker:
    ...
    def _generate_personalized_email(self, site_url: str, variables: Dict[str, Any]) -> str:
        ...
        # Use the provided recent work URL from variables if available, otherwise use the target's recent post
        recent_work_url = variables.get('recent_work_url')
        if not recent_work_url:
            # If no recent_work_url was provided in variables, use our own recent post
            our_blog_url = variables.get('our_blog_url')
            if our_blog_url:
                recent_work_url = self._fetch_recent_post(our_blog_url)
...
if __name__ == "__main__":
    ...
    variables = {
        ...
        "our_blog_url": "https://ruckquest.com"  # Using the same URL for testing
    }
