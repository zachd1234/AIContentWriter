...
class EmailCreator:
    ...
    def create_advanced_email(self, url: str, site_id: int) -> Optional[Dict[str, str]]:
        ...
        try:
            # Get pitch data from the post pitch API first
            pitch_data = self._get_post_pitch(url)
            ...
            # Check specifically for "NO EMAIL FOUND" case
            if recipient_email == "NO EMAIL FOUND":
                logger.warning(f"No email found for {url}, skipping personalization")
                return None
            
            # Skip if any required field is missing from post pitch
            if not all([subject, post_pitch_personalization, recipient_email]):
                logger.warning(f"Missing required data from post pitch for {url}")
                return None
            
            # Only proceed with the expensive Gemini call if we have valid post pitch data
            template_maker_instance = template_maker.TemplateMaker()
            advanced_personalization = template_maker_instance.generate_advanced_personalized_email(url, site_id)
            
            if not advanced_personalization:
                logger.warning(f"Failed to generate advanced personalization for {url}")
                return None
...
if __name__ == "__main__":
    ...
    # Now test the advanced email creation
    print("\nTesting advanced email creation:")
    email = creator.create_advanced_email(test_urls[0], 1)
    
    print(f"\nCreated advanced email for {test_urls[0]}")
