...
class MockDatabaseService:
    ...
    def find_recent_emails_by_recipient(self, recipient, site_id=None):
        """Mock method that returns a fake email record"""
        print(f"MOCK DB: Finding recent emails for recipient: {recipient}")
    ...
    def update_email_status(self, email_id, status, response_text=None):
        """Mock method to update email status"""
        print(f"MOCK DB: Updating email {email_id} status to '{status}'")
        if response_text:
            print(f"MOCK DB: Response text: {response_text[:50]}...")
        return True
