...
class ProspectGenerator:
    ...
    def structure_categories_to_search_terms(self, categories_text: str, core_phrase: str) -> List[Dict[str, str]]:
        ...
        The search query should be specific enough to find relevant websites but general enough to return a good number of results.
        """
...
def main():
    ...
    # Group prospects by category for better visualization
    prospects_by_category = {}
    for prospect in llm_prospects:
        category = prospect.get('category', 'Uncategorized')
        if category not in prospects_by_category:
            prospects_by_category[category] = []
        prospects_by_category[category].append(prospect)
    
    # Print the list of LLM-based prospects by category
    print("\nLLM-Based Prospect List (By Category):")
    for category, prospects in prospects_by_category.items():
        print(f"\n== {category} ({len(prospects)} prospects) ==")
        print(f"Search term: {prospects[0]['search_term']}")
        for i, prospect in enumerate(prospects[:5], 1):  # Show top 5 per category
