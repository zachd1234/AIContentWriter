...
class ContentGenerator:
    def __init__(self):
        ...
        search_tool = Tool(
            ...
            description="Searches Google for current information about the topic"
        )

        # Initialize the agent
        self.agent = initialize_agent(
            tools=[research_tool, search_tool],
            llm=self.llm,
    ...
    def generate_blog_post(self, keyword: str) -> dict:
        ...
        """
        try:
            # Let the agent plan and execute the content creation
            agent_prompt = f"""Create a high-quality blog post about "{keyword}". 
            Follow these steps:
            1. Research the topic thoroughly
            2. Identify key points and statistics
            3. Find authoritative sources
            4. Structure and write the content
    ...
    def fetch_serp_results(self, keyword: str) -> dict:
        """
        Fetches and analyzes top 10 search results using Serper API
        """
        try:
            ...
            # Have the model analyze the results
            analysis_prompt = f"""Analyze these top 10 search results for "{keyword}":
