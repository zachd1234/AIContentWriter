...
class DatabaseService:
    ...
    def pop_next_urls(self, max_count: int) -> List[Dict[str, Any]]:
        """
        Retrieve the top URLs from the outreach list, move them to the back of the list,
        and return them.
        ...
        try:
            ...
            cursor.execute(
                """
                SELECT id, url, email, site_id, metadata, priority
                FROM outreach_urls
            ...
            for row in results:
                url_id, url, email, site_id, metadata, priority = row
            ...
            # Get the minimum priority to place these at the back
            cursor.execute("SELECT MIN(priority) FROM outreach_urls")
            min_priority = cursor.fetchone()[0] or 0
            
            # Update these URLs with the lowest priority to move them to the back
            placeholders = ','.join(['?'] * len(url_ids))
            cursor.execute(
                f"""
                UPDATE outreach_urls
                SET priority = ?
                WHERE id IN ({placeholders})
                """,
                [min_priority - 1] + url_ids
            )
