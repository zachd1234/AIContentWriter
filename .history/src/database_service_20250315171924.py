...
class DatabaseService:
    ...
    def get_urls_by_category(self, category: str, site_id: int, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        Then move these URLs to the back of the outreach_urls list.
        ...
        try:
            ...
            # Process each URL - move it to the back of the list
            for row in results:
                ...
                cursor.execute(
                    "DELETE FROM outreach_urls WHERE id = %s",
                    (url_id,)
                )
                
                # Re-add the URL at the end of the list
                cursor.execute(
                    "INSERT INTO outreach_urls (url, site_id, website_category) VALUES (%s, %s, %s)",
                    (url, site_id, website_category)
                )
            ...
            logger.info(f"Retrieved and moved {len(urls)} URLs for category '{category}' and site ID {site_id}")
            return urls
