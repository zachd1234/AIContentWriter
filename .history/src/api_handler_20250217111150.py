...
class ContentAPIHandler:
    ...
    async def generate_complete_post(self, keyword: str) -> dict:
        ...
        try:
            ...
            blog_post = await self.blog_generator.create_complete_post(keyword)
            
            # Add media content
            post_with_media = await self.media_handler.enhance_post(blog_post)
            ...
            return {
                "status": "success",
                "data": post_with_media, #change back to final post
                "keyword": keyword
            }
