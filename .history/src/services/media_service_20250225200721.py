...
class GetImgAIClient:
    def __init__(self, base_url: str):
        ...
        self.llm = ChatGoogleGenerativeAI(
            model="models/gemini-2.0-pro-exp-02-05",  # Updated with full model path
            temperature=0.7,
...
class PostWriterV2:
    ...
    def extract_json_from_agent_output(self, text: str) -> str:
        """
        Extracts JSON from agent output, handling various formats and edge cases
        
        Args:
            text (str): The raw agent output text
            
        Returns:
            str: The extracted JSON string or empty array if no valid JSON found
        """
        try:
            ...
            if text.strip().startswith('[') and text.strip().endswith(']'):
                print("✅ Found direct JSON array")
                return text.strip()
            
            # Case 2: JSON in code blocks (with any number of backticks)
            code_block_match = re.search(r'```+(?:json)?\s*([\s\S]*?)\s*```+', text, re.DOTALL)
            if code_block_match:
                print("✅ Found JSON in code block")
                return code_block_match.group(1).strip()
            
            # Case 3: "Thought:Thought:" pattern (seen in logs)
            thought_thought_match = re.search(r'Thought:Thought:.*?(\[\s*\{.*?\}\s*\])', text, re.DOTALL)
            if thought_thought_match:
                print("✅ Found JSON after Thought:Thought: pattern")
                return thought_thought_match.group(1).strip()
            
            # Case 4: JSON after Thought: prefix
            thought_match = re.search(r'Thought:.*?(\[\s*\{.*?\}\s*\])', text, re.DOTALL)
            if thought_match:
                print("✅ Found JSON after Thought: prefix")
                return thought_match.group(1).strip()
            
            # Case 5: JSON in Thought: code block
            thought_code_match = re.search(r'Thought:.*?```+(?:json)?\s*([\s\S]*?)\s*```+', text, re.DOTALL)
            if thought_code_match:
                print("✅ Found JSON in Thought: code block")
                return thought_code_match.group(1).strip()
            
            # Case 6: JSON after Final Answer: prefix
            final_answer_match = re.search(r'Final Answer:.*?(\[\s*\{.*?\}\s*\])', text, re.DOTALL)
            if final_answer_match:
                print("✅ Found JSON after Final Answer: prefix")
                return final_answer_match.group(1).strip()
            
            # Case 7: Any JSON array in the text (fallback)
            json_array_match = re.search(r'\[\s*\{.*?\}\s*\]', text, re.DOTALL)
            if json_array_match:
                print("✅ Found JSON array (fallback method)")
                return json_array_match.group(0).strip()
            
            print("❌ Could not find any JSON in the agent output")
            return "[]"
        except Exception as e:
    ...
    def enhance_post(self, blog_post: str) -> str:
        ...
        try:
            ...
            try:
                ...
                return self.extract_json_from_agent_output(output_text)
                
                # Validate the JSON format
                #return self.validate_json_response(json_str)
    ...
    def populate_media_in_html(self, html_content: str, media_json: str) -> str:
        """
        Populates the HTML content with media from the JSON
        
        Args:
            html_content (str): The HTML content
            media_json (str): The media JSON string
            
        Returns:
            str: The HTML content with media inserted
        """
        try:
...
def main():
    ...
    media_json = post_writer.enhance_post(sample_post)
    
    # Then populate the HTML with the media
    enhanced_post = post_writer.populate_media_in_html(sample_post, media_json)
