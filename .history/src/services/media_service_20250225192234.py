...
class PostWriterV2:
    ...
    def validate_json_response(self, agent_output: str) -> str:
        ...
        try:
            ...
            # Handle the specific case of four backticks
            if cleaned_output.startswith("````json"):
                cleaned_output = cleaned_output.replace("````json", "", 1)
                if cleaned_output.endswith("````"):
                    cleaned_output = cleaned_output[:-4]
            
            # Also handle the standard three backticks case
            if cleaned_output.startswith("```json"):
                cleaned_output = cleaned_output.replace("```json", "", 1)
                if cleaned_output.endswith("```"):
