...
class LinkingAgent:
    ...
    def suggest_internal_links(self, post_content: str) -> list:
        ...
        try:
            ...
            # Generate structured response - this is how it's done in MediaService
            response = model.generate_content(
                contents=prompt,
                generation_config=None,
                safety_settings=None,
                stream=False,
                tools=[{"schema_version": "1.0.0", "schema": response_schema}]
            )
            
            # Extract suggestions from structured response
            suggestions = response.candidates[0].content.parts[0].function_call.args["items"]
    ...
    def process_content_with_links(self, content: str, base_url: str) -> str:
        ...
        try:
            ...
            for suggestion in suggestions:
                ...
                # Find the first occurrence of the anchor text that's not already linked
                start_pos = 0
                while True:
                    pos = modified_content.find(anchor_text, start_pos)
                    if pos == -1:
                        break
                    ...
                    if not overlap:
                        ...
                        modified_content = modified_content[:pos] + html_link + modified_content[pos + len(anchor_text):]
                        
                        # Add this range to linked_ranges
                        linked_ranges.append((pos, pos + len(html_link)))
                        
                        # We've added this link, so move on to the next suggestion
                        break
