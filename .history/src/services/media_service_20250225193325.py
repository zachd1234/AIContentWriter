        ...
        try:
            ...
            try:
                ...
                if json_array_match:
                    ...
                    # Last resort: Ask the agent to fix the JSON
                    print("🔄 Attempting to fix JSON with the agent")
                    try:
                        # This is where we would use StrictJSON's error correction
                        # For now, we'll just return an empty array
                        print("❌ Could not fix JSON")
                        return "[]"
                    except Exception as e3:
                        print(f"❌ Error fixing JSON: {str(e3)}")
                        return "[]"
