...
class PostWriterV2:
    ...
    def validate_json_response(self, agent_output: str) -> str:
        ...
        try:
            ...
            thought_match = re.search(r'Thought:.*?```+(?:json)?\s*([\s\S]*?)\s*```', cleaned_output, re.DOTALL)
            if thought_match:
                print(f"🔍 Detected JSON in Thought: block")
                cleaned_output = thought_match.group(1).strip()
                print(f"✅ Successfully extracted JSON from Thought: block")
