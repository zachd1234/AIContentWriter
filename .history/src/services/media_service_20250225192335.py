...
class PostWriterV2:
    ...
    def validate_json_response(self, agent_output: str) -> str:
        ...
        try:
            ...
            # Handle markdown code blocks (both three and four backticks)
            if "````json" in cleaned_output or "```json" in cleaned_output:
                ...
                if json_match:
                    ...
                    print(f"⚠️ Could not extract JSON from code block using regex")
            
            # Also handle the standard three backticks case as fallback
            ...
            try:
                ...
                print(f"❌ Initial JSON parsing error: {str(e)}")
                ...
                if json_array_match:
                    ...
                    try:
                        ...
                        print(f"❌ JSON parsing error: {str(e2)}")
                        return "[]"
                else:
            ...
            for item in media_items:
                if "type" not in item:
                    print(f"❌ Skipping item without type: {item}")
                    continue
                    
                if item["type"] == "youtube" and "videoId" not in item:
                    print(f"❌ Skipping YouTube item without videoId: {item}")
                    continue
                    
                if item["type"] == "image" and "prompt" not in item:
                    print(f"❌ Skipping image item without prompt: {item}")
                    continue
