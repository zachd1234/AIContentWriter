    ...
    def process_content_with_links(self, content: str, base_url: str) -> str:
        ...
        try:
            ...
            for suggestion in suggestions:
                ...
                # Use regex to find the anchor text that is not inside HTML tags
                # This pattern looks for the anchor text that is not inside < and >
                pattern = re.compile(f'(?<![<>])(?<![<][^>]*){re.escape(anchor_text)}(?![^<]*[>])')
                
                # Find first occurrence that matches our pattern
                ...
                if match:
                    ...
                    modified_content = modified_content[:start] + html_link + modified_content[end:]
