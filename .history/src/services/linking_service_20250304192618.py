...
class LinkingAgent:
    ...
    def suggest_internal_links(self, post_content: str) -> list:
        ...
        try:
            ...
            suggestions = response.candidates[0].content.parts[0].function_response.response["items"]
        ...
        try:
            ...
            for suggestion in suggestions:
                ...
                while True:
                    ...
                    start_pos = index + len(anchor_text)
            ...
            for original_index, suggestion in suggestions_with_positions:
                ...
                if region_available:
                    ...
                    new_linked_chars = [False] * len(content_chars)
                    
                    # Copy the existing linked status for the part before the link
                    for i in range(min(index, len(linked_chars))):
                        new_linked_chars[i] = linked_chars[i]
                    
                    # Mark the newly linked region
                    for i in range(index, index + len(html_link)):
                        new_linked_chars[i] = True
                    
                    # Copy the existing linked status for the part after the link
                    for i in range(index + anchor_length, len(linked_chars)):
                        new_pos = i + (len(html_link) - anchor_length)
                        if new_pos < len(new_linked_chars):
                            new_linked_chars[new_pos] = linked_chars[i]
                    
                    # Update linked_chars with the new array
                    linked_chars = new_linked_chars
