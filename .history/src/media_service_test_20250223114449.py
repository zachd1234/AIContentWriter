...
class GetImgAIClient:
    def __init__(self):
        ...
        # Initialize Gemini for prompt enhancement and vision tasks
    ...
    def evaluate_images(self, image_urls: list[str], original_prompt: str) -> str:
        """Uses vision model to select the best image."""
        evaluation_prompt = f"""Evaluate these images based on the original prompt: "{original_prompt}"

        Consider:
        1. How well does each image match the intended concept?
        2. Visual quality and coherence
        3. Effectiveness for educational/illustrative purposes
        4. Aesthetic appeal and professionalism

        Return ONLY the number (1, 2, or 3) of the best image."""
        
        # Create a combined evaluation of all images
        responses = []
        ...
        # Parse responses to find the best image number
        best_image_num = int(min(responses))  # You'll need to adjust parsing based on actual response format
        return image_urls[best_image_num - 1]

    def generate_image(self, prompt: str, width=1024, height=1024, steps=4, base_url: str = "https://ruckquest.com") -> str:
        ...
        try:
            ...
            if not image_urls:
                return "❌ No images were generated successfully."

            # Step 3: Use vision model to select best image
            best_image_url = self.evaluate_images(image_urls, prompt)
            print("✅ Selected best image URL:", best_image_url)
            ...
            try:
                ...
                media_id = wp_handler.upload_image_from_url(best_image_url)
...
class PostWriterV2:
    def __init__(self):
        ...
        def generate_image_with_url(prompt: str, base_url: str = "https://ruckquest.com") -> str:
