...
def suggest_internal_links(post_url, post_content):
    """
    Suggests internal links for a given post by analyzing other posts from the sitemap
    
    Args:
        post_url (str): The URL of the current post
    ...
    try:
        ...
        # Remove the current post from potential links
        potential_links = [post for post in all_posts if post['loc'] != post_url]
        
        suggestions = []
        
        # Extract meaningful words from the post URLs (excluding common WordPress parts)
        for target_post in potential_links:
