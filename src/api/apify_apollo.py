...
class ApifyApolloClient:
    ...
    def get_employee_name_from_url(self, url: str) -> Optional[str]:
        """
        Get an employee name from a URL using Apify's Apollo.io scraper.
        ...
        # Search for people by domain
        people = self.search_people_by_domain(domain, limit=1)
        ...
        if not name:
            print(f"Person found but no name available")
            return None
...
if __name__ == "__main__":
    ...
    print("\nTesting find_email_by_name:")
    test_name = "Sam Altman"
    test_domain = "openai.com"
    test_company = "OpenAI"
    
    emails = client.find_email_by_name(test_name, test_domain, test_company)
    
    if emails:
        print(f"Found {len(emails)} email(s) for {test_name} at {test_domain}:")
        for email in emails:
            print(f"  - {email}")
    else:
        print(f"No emails found for {test_name} at {test_domain}")
    
    # Test getting an employee name from a URL
    print("\nTesting get_employee_name_from_url:")
    test_urls = ["https://openai.com", "https://microsoft.com", "https://google.com"]
    
    for url in test_urls:
        print(f"\nGetting employee name from URL: {url}")
        name = client.get_employee_name_from_url(url)
        
        if name:
            print(f"Found employee name: {name}")
        else:
            print(f"No employee name found for URL: {url}") 