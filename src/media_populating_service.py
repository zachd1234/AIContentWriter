...
- Limit to 1-2 videos maximum to avoid overwhelming the reader"""
    ...
    def populate_media_in_html(self, html_content: str) -> str:
        ...
        try:
            ...
            print("🔍 Raw media suggestions:", media_json)
            
            # Handle case where media_json is already a list/dict
            if isinstance(media_json, (list, dict)):
                media_placements = media_json
            else:
                try:
            ...
            for placement in media_placements:
                ...
                if media_type == 'image':
                    # Clean up the URL - remove any duplicate WordPress prefixes
                    if 'wp-content/uploads/' in media_url:
                        clean_url = media_url.split('wp-content/uploads/')[-1]
                        final_url = f"https://ruckquest.com/wp-content/uploads/{clean_url}"
                    else:
                        final_url = media_url
                    
                    media_html = f'<img src="{final_url}" alt="{placement.get("description", "")}" style="max-width: 100%; height: auto;">'
                else:  # video
                    # Extract video ID from YouTube URL
                    if 'youtube.com' in media_url:
                        if 'watch?v=' in media_url:
                            video_id = media_url.split('watch?v=')[-1].split('&')[0]
                        elif 'embed/' in media_url:
                            video_id = media_url.split('embed/')[-1].split('?')[0]
                        else:
                            video_id = media_url.split('/')[-1].split('?')[0]
                        
                        media_html = f'<iframe style="aspect-ratio: 16 / 9; width: 100%" src="https://www.youtube.com/embed/{video_id}" title="YouTube video" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen></iframe>'
                    else:
                        # For non-YouTube videos, just embed the URL directly
                        media_html = f'<a href="{media_url}" target="_blank">Watch Video</a>'
                ...
                if insert_before in html_content:
                    ...
                    print(f"⚠️ Could not find insertion point: {insert_before[:50]}...")
