...
class ControlPanel:
    ...
    def run_advanced_outreach_campaign(self, site_id: int, post_url: str, post_title: str) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with campaign results
        """
        logger.info(f"Starting advanced outreach campaign for site ID: {site_id}, post: {post_title}")
        
        # Step 1: Get prospects from the prospect generator
        ...
        logger.info(f"Retrieved {len(prospects)} prospects from best category: {best_category}")
        print(f"\n===== PROSPECTS RETRIEVED =====")
        print(f"Best category: {best_category}")
        for i, prospect in enumerate(prospects):
            print(f"Prospect {i+1}: {prospect}")
        print("==============================\n")
        
        if not prospects:
            return {"status": "failed", "reason": "No prospects available", "emails_sent": 0}
        
        # Step 2: Create advanced personalized emails one by one
        print(f"\n===== CREATING ADVANCED EMAILS =====")
        emails = []
        
        for prospect in prospects:
            # Extract just the URL from the prospect object
            url = prospect['url'] if isinstance(prospect, dict) else prospect
            
            print(f"Creating advanced email for: {url}")
            email = self.email_creator.create_advanced_email(url, site_id)
            if email:
                emails.append(email)
                print(f"✅ Successfully created email for {url}")
            else:
                print(f"❌ Failed to create email for {url}")
        
        logger.info(f"Created {len(emails)} advanced personalized emails")
        print(f"Created {len(emails)} advanced personalized emails")
        ...
        if emails:
            ...
            for i, email in enumerate(emails):
                ...
                print(f"To: {email.get('email', 'No email address')}")
                print(f"Subject: {email.get('subject', 'No subject')}")
                print(f"Body:\n{email.get('body', 'No body')[:200]}...")  # Show just the first 200 chars
                print("----------------------------")
        else:
            print("No emails were created!")
        print("==============================\n")
